...
def detect_anomalies(df: pd.DataFrame) -> tuple[pd.DataFrame, list[dict]]:
                ...
                summary.append(
                    {
                        ...
                        "count": count,
                        "severity": (
                            "high" if col in ("Contract_ID", "Account_ID") else "medium"
                        ),
    ...
    data_cols = [
        c for c in df.columns if not c.startswith("meta_") and c != "_anomaly_flags"
    ]
    if data_cols:
    ...
    logger.info(
        "Anomaly detection: %d/%d rows flagged, %d issue types.",
        total_flagged,
        n,
        len(summary),
    )
