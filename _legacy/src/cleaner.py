...
class Cleaner:
    ...
    def sanitize_errors(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if count > 0:
                ...
                total_errors += count
                logger.warning(
                    "Sanitized %d Excel error(s) in column '%s'.",
                    count,
                    col,
                )
    ...
    def normalize_dates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        range_cols = [
            c for c in df.columns if "range" in c.lower() and "date" in c.lower()
        ]
    ...
    def _split_date_range_column(self, df: pd.DataFrame, col: str) -> pd.DataFrame:
        ...
        if "Effective_Date" not in df.columns:
            ...
            df["Effective_Date"] = (
                df["Effective_Date"].dt.strftime("%Y-%m-%d").replace("NaT", pd.NA)
            )
        if "Expiry_Date" not in df.columns:
            df["Expiry_Date"] = pd.to_datetime(end_dates, errors="coerce")
            df["Expiry_Date"] = (
                df["Expiry_Date"].dt.strftime("%Y-%m-%d").replace("NaT", pd.NA)
            )

        # Drop the original range column
        df = df.drop(columns=[col])
        logger.info(
            "Split date range column '%s' into 'Effective_Date' and 'Expiry_Date'.", col
        )
    ...
    def _clean_date_column(self, df: pd.DataFrame, col: str) -> pd.DataFrame:
        ...
        for idx, val in df[col].items():
            ...
            try:
                ...
                logger.warning(
                    "Unparseable date '%s' in column '%s' (row %s).", val_str, col, idx
                )
                parsed.append(pd.NaT)
    ...
    def scrub_financials(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        financial_cols = [
            c
            for c in df.columns
            if c in _CANONICAL_FINANCIAL
            or any(
                kw in c.lower()
                for kw in ("value", "cost", "price", "amount", "acv", "tcv", "arr")
            )
        ]
        ...
        if "Total_Value" in df.columns and "Term_Months" in df.columns:
            ...
            mask = (
                df["ACV"].isna() & df["Total_Value"].notna() & df["Term_Months"].notna()
            )
            if mask.any():
                ...
                logger.info(
                    "Computed ACV for %d rows from Total_Value / Term_Months.",
                    mask.sum(),
                )
        ...
        for col in df.columns:
            ...
            if df[col].dtype == object:
                ...
                if unique_lower and unique_lower.issubset(BOOL_MAP.keys()):
                    df[col] = df[col].apply(
                        lambda v: (
                            BOOL_MAP.get(str(v).strip().lower(), v)
                            if pd.notna(v)
                            else v
                        )
                    )
