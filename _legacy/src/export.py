...
def export_rich_excel(
    ...
    logger.info(
        "Rich Excel exported: %s (%d rows, 4 sheets)", output_path, len(master_df)
    )
    return output_path
...
def _write_audit_sheet(ws, audit_data: dict, mapping_log: list[dict]):
    ...
    for mapping in mapping_log:
        ws.cell(row=row, column=1, value=mapping.get("original", "")).border = (
            _THIN_BORDER
        )
        ws.cell(row=row, column=2, value=mapping.get("canonical", "—")).border = (
            _THIN_BORDER
        )
        ...
        ws.cell(row=row, column=4, value=mapping.get("source", "")).border = (
            _THIN_BORDER
        )
        row += 1
...
def _write_vendor_sheet(ws, vendor_clusters: list[dict]):
    ...
    if not vendor_clusters:
        ws.cell(row=row, column=1, value="No vendor clusters found.").font = Font(
        ...
        for cluster in vendor_clusters:
            ...
            ws.cell(row=row, column=1, value=canonical).font = Font(
                bold=True, color="6366F1"
            )
            ws.cell(row=row, column=1).border = _THIN_BORDER
            ws.cell(row=row, column=2, value=", ".join(non_canonical)).border = (
                _THIN_BORDER
            )
...
def _write_quality_sheet(ws, df: pd.DataFrame):
    ...
    total_rows = len(df)
    if total_rows == 0:
        ws.cell(row=row, column=1, value="No data to analyze.").font = Font(
            italic=True, color="999999"
        )
        return
    ...
    overall_pct = (
        round(overall_non_null / overall_total * 100, 1) if overall_total > 0 else 0
    )

    ws.cell(row=row, column=1, value="OVERALL").font = Font(bold=True, size=12)
    ws.cell(row=row, column=4, value=overall_pct / 100).number_format = (
