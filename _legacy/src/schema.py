...
def fuzzy_map_headers(
    ...
    for col in df.columns:
        ...
        if match:
            ...
            if _validate_fuzzy_match(col_lower, matched_alias, canonical, score):
                ...
                mapped_canonicals.add(canonical)
                logger.info(
                    "Fuzzy match: '%s' → '%s' (score=%d via alias '%s')",
                    col,
                    canonical,
                    score,
                    matched_alias,
                ...
                logger.info(
                    "Rejected fuzzy match: '%s' → '%s' (score=%d) — semantics mismatch.",
                    col,
                    canonical,
                    score,
                )
    ...
    for col in still_unmapped:
        ...
        if detected and detected not in mapped_canonicals:
            ...
            logger.info(
                "Content-detected: '%s' → '%s' (based on cell values)", col, detected
            )
        else:
            unmapped.append(col)
    ...
    for orig, canon in list(rename_map.items()):
        if canon in seen:
            ...
            logger.warning(
                "Duplicate canonical '%s' — renaming '%s' to '%s'.",
                canon,
                orig,
                new_name,
            )
