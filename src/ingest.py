...
def load_excel(filepath: str) -> list[pd.DataFrame]:
    ...
    for sheet_name in xls.sheet_names:
        try:
            ...
            if df.empty:
                logger.warning("Sheet '%s' in %s is empty after cleanup — skipping.", sheet_name, filepath.name)
                continue
            df["meta_source_tab"] = sheet_name
            df["meta_source_file"] = filepath.name
            frames.append(df)
            logger.info("Loaded %d rows from sheet '%s' (%s).", len(df), sheet_name, filepath.name)
        except Exception as exc:
            logger.error("Error reading sheet '%s' in %s: %s — skipping.", sheet_name, filepath.name, exc)
    ...
    for entry in sorted(dir_path.iterdir()):
        ...
        try:
            if entry.suffix.lower() == ".csv":
                ...
                if df.empty:
                    logger.warning("CSV file %s is empty after cleanup — skipping.", entry.name)
                    continue
...
def _clean_frame(df: pd.DataFrame, sheet: str, filename: str) -> pd.DataFrame:
    ...
    if unnamed_cols:
        ...
        logger.info(
            ...
            len(unnamed_cols), sheet, filename,
        )
    ...
    if all_nan_cols:
        ...
        logger.info(
            ...
            len(all_nan_cols), sheet, filename, all_nan_cols,
        )
    ...
    if dropped_cols or dropped_rows:
        logger.info(
            ...
            sheet, filename, dropped_cols, dropped_rows, len(df), len(df.columns),
        )
...
def _expand_composite_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if new_cols:
        ...
        logger.info("Dropped %d composite column(s) after extraction.", len(composite_cols))
