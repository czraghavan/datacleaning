...
META_COLS = {"meta_source_tab", "meta_source_file", "Vendor_Canonical", "Vendor_Cluster_ID"}
...
def concat_all(dfs: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    if all_nan:
        ...
        logger.info("Post-concat: dropped %d all-NaN columns: %s", len(all_nan), all_nan)
    ...
    logger.info("Concatenated %d frames → %d total rows, %d columns.", len(dfs), len(combined), len(combined.columns))
    return combined
...
def merge_quote_lines(
    ...
    if df_keyed.empty:
        logger.info("No rows with valid grouping key (%s) — skipping merge.", group_cols)
        return df
...
_ID_KEYWORDS = [
    "contract_id", "contract id", "quote_id", "quote id",
    "deal_id", "deal id", "opp_id", "opp id", "opportunity_id",
    "order_id", "order id", "agreement_id", "po_number",
    "subscription_id", "record_id", "ref",
    "contract #", "quote #", "deal #", "opp #", "order #",
    "contract_number", "quote_number", "deal_number",
]

# Keywords for auto-detecting close-date columns
_DATE_KEYWORDS = [
    "close_date", "close date", "closed_date", "closed date",
    "booking_date", "booking date", "win_date", "won_date",
    "signed_date", "executed_date", "creation_date",
