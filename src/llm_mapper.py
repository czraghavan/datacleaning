...
CANONICAL_CATEGORIES = [
    "Contract_ID", "Contract_Close_Date",
    "Vendor", "ACV", "Total_Value", "Term_Months", "Currency", "Quantity",
    "Unit_Price", "Discount",
    "Effective_Date", "Renewal_Date", "Expiry_Date",
    "Auto_Renew", "Close_Year", "Fiscal_Period", "Account_Segment",
    "Product", "Department", "Owner", "Status", "Opportunity_Type",
    "Notes", "Category",
]
...
def llm_detect_columns(
    ...
    for col in unmapped_cols:
        ...
        samples_str = ", ".join(f'"{s}"' for s in samples) if samples else "(no samples)"
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            resp = client.post(
                ...
                json={
                    "model": "gpt-4o-mini",
                    "messages": [
                        {"role": "system", "content": "You are a data classification expert. Respond only with valid JSON."},
        ...
        for col, cat in result.items():
            if cat in CANONICAL_CATEGORIES and col in unmapped_cols:
                ...
                logger.debug("LLM returned invalid category '%s' for '%s' — skipping.", cat, col)
